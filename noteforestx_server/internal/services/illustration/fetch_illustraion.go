...
func (this *IllustrationService) GetIllustrationList(ctx *gin.Context) {
	...
	db := this.Db.Model(&models.Illustration{}).
		...
		Preload("Images", func(tx *gorm.DB) *gorm.DB {
			return tx.Where("`order` = 0") // 只加载第一张图
		})
	...
	// 查询总数（单独用干净的 db，不带 group/order/limit）
	var total int64
...
func (this *IllustrationService) FetchIllustrationsByAuthorId(ctx *gin.Context) {
	authorId, err := this.utils.GetAndParseParamUuid("author_id", ctx)
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{"message": "author_id err: " + err.Error()})
		return
	}

	var illustrationList []models.Illustration
	this.Db.Model(&models.Illustration{}).
		Preload("Tags").
		Preload("Images", func(db *gorm.DB) *gorm.DB {
			return db.Order("`order` ASC").Limit(1)
		}).
		Where("author_id = ?", authorId).Find(&illustrationList)
